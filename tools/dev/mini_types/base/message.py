...
class BaseMessageMin(MessagesMessage, ABC):
    ...
    async def edit(self, message: str = ''):
        return await self.ctx_api.messages.edit(message_id = self.id, message = message, peer_id = self.peer_id, keep_forward_messages = True, keep_snippets = True, dont_parse_links = False)
