...
from vkbottle.api.vk_api.vk_api import VkApiGroup
...
class Bot(ABCFramework):
    def __init__(
        ...
        self.vk_api = VkApiGroup(token = token).get_api()
        if id:
            self.api.id = id
        else:
            self.api.id = self.api.groups.getById()[0]['id']
        self.id = self.api.id
        self.api.vk_api = self.vk_api
